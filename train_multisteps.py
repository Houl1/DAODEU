...
from models.model import Mymodelnew
...
if __name__ == "__main__":
    ...
    dataset = MyDataset(args, graphs[:-4], adjs[:-4], context_pairs_train)
    ...
    patient = 0
    best_epoch_mae = [0,0,0,0,0]
    best_epoch_ap = [0,0,0,0,0]
    best_epoch_mse = [0,0,0,0,0]
    for epoch in range(args.epochs):
        ...
        model.eval()
        emb1 = model(feed_dict["graphs"],feed_dict["global_pyg"])[0][-5].detach().cpu().numpy()
        # activity1 = model(feed_dict["graphs"],feed_dict["global_pyg"])[-1][-5].detach().cpu().numpy()
        ...
        # mae_score1,mse_score1 = get_score_t(feed_dict["graphs"][-5].detach().cpu().node_activity.numpy(),activity1)
        
        emb2 = model(feed_dict["graphs"],feed_dict["global_pyg"])[0][-4].detach().cpu().numpy()
        # activity2 = model(feed_dict["graphs"],feed_dict["global_pyg"])[-1][-4].detach().cpu().numpy()
        # mae_score,recall_score,mse_score,f1_score = get_score_t(feed_dict["graphs"][-1].detach().cpu().node_activity.numpy(),activity)
        ...
        # mae_score2,mse_score2 = get_score_t(feed_dict["graphs"][-4].detach().cpu().node_activity.numpy(),activity2)
        
        emb3 = model(feed_dict["graphs"],feed_dict["global_pyg"])[0][-3].detach().cpu().numpy()
        # activity3 = model(feed_dict["graphs"],feed_dict["global_pyg"])[-1][-3].detach().cpu().numpy()
        ...
        # mae_score3,mse_score3 = get_score_t(feed_dict["graphs"][-3].detach().cpu().node_activity.numpy(),activity3)
        
        emb4 = model(feed_dict["graphs"],feed_dict["global_pyg"])[0][-2].detach().cpu().numpy()
        # activity4 = model(feed_dict["graphs"],feed_dict["global_pyg"])[-1][-2].detach().cpu().numpy()
        ...
        # mae_score4,mse_score4 = get_score_t(feed_dict["graphs"][-2].detach().cpu().node_activity.numpy(),activity4)
        
        emb5 = model(feed_dict["graphs"],feed_dict["global_pyg"])[0][-1].detach().cpu().numpy()
        # activity5 = model(feed_dict["graphs"],feed_dict["global_pyg"])[-1][-1].detach().cpu().numpy()
        ...
        # mae_score5,mse_score5 = get_score_t(feed_dict["graphs"][-1].detach().cpu().node_activity.numpy(),activity5)
        ...
        if epoch_auc_val1 > best_epoch_val:
            ...
            # best_epoch_mae = [mae_score1,mae_score2,mae_score3,mae_score4,mae_score5]
            # best_epoch_mse = [mse_score1,mse_score2,mse_score3,mse_score4,mse_score5]
    ...
    emb1 = model(feed_dict["graphs"],feed_dict["global_pyg"])[0][-5].detach().cpu().numpy()
    # activity1 = model(feed_dict["graphs"],feed_dict["global_pyg"])[-1][-5].detach().cpu().numpy()
    val_results1, test_results1, _, _,ap1 = evaluate_classifier(train_edges_pos[0],
                                                          train_edges_neg[0],
                                                          val_edges_pos[0],
                                                          val_edges_neg[0],
                                                          test_edges_pos[0],
    ...
    emb2 = model(feed_dict["graphs"],feed_dict["global_pyg"])[0][-4].detach().cpu().numpy()
    # activity2 = model(feed_dict["graphs"],feed_dict["global_pyg"])[-1][-4].detach().cpu().numpy()
    val_results2, test_results2, _, _,ap2 = evaluate_classifier(train_edges_pos[1],
                                                          train_edges_neg[1],
                                                          val_edges_pos[1],
                                                          val_edges_neg[1],
                                                          test_edges_pos[1],
    ...
    emb3 = model(feed_dict["graphs"],feed_dict["global_pyg"])[0][-3].detach().cpu().numpy()
    # activity3 = model(feed_dict["graphs"],feed_dict["global_pyg"])[-1][-3].detach().cpu().numpy()
    val_results3, test_results3, _, _,ap3 = evaluate_classifier(train_edges_pos[2],
                                                          train_edges_neg[2],
                                                          val_edges_pos[2],
                                                          val_edges_neg[2],
                                                          test_edges_pos[2],
    ...
    emb4 = model(feed_dict["graphs"],feed_dict["global_pyg"])[0][-2].detach().cpu().numpy()
    # activity4 = model(feed_dict["graphs"],feed_dict["global_pyg"])[-1][-2].detach().cpu().numpy()
    val_results4, test_results4, _, _,ap4 = evaluate_classifier(train_edges_pos[3],
                                                          train_edges_neg[3],
                                                          val_edges_pos[3],
                                                          val_edges_neg[3],
                                                          test_edges_pos[3],
    ...
    emb5 = model(feed_dict["graphs"],feed_dict["global_pyg"])[0][-1].detach().cpu().numpy()
    # activity5 = model(feed_dict["graphs"],feed_dict["global_pyg"])[-1][-1].detach().cpu().numpy()
    val_results5, test_results5, _, _,ap5 = evaluate_classifier(train_edges_pos[4],
                                                          train_edges_neg[4],
                                                          val_edges_pos[4],
                                                          val_edges_neg[4],
                                                          test_edges_pos[4],
    ...
    auc_test = [auc_test1,auc_test2,auc_test3,auc_test4,auc_test5]
    ap = [ap1,ap2,ap3,ap4,ap5]
    if auc_test1<best_epoch_test[0]:
        auc_test=best_epoch_test
    if ap1<best_epoch_ap[0]:
        ap=best_epoch_ap
