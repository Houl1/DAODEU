...
if __name__ == "__main__":
    ...
    dataset = DynamicsDataset(args, G, A,x0)
    dataloader = DataLoader(dataset,
                            batch_size=args.n,
                            shuffle=True,
                            num_workers=2,
                            collate_fn=DynamicsDataset.collate_fn)
    model = DynamicsModel(args, num_feat=x0.shape[1], time_length=args.time_steps).to(device)
    ...
    for epoch in range(args.epochs+1):
        ...
        for idx, feed_dict in enumerate(dataloader):
            ...
            opt.zero_grad()
            pred_y = model(feed_dict["graph"],feed_dict["global_pyg"],t_train)
            # print(pred_y.shape)
            ...
            if epoch % args.test_freq == 0:
                with torch.no_grad():
                    pred_y = model(feed_dict["graph"],feed_dict["global_pyg"], t).squeeze().t()  # odeint(model, true_y0, t)
